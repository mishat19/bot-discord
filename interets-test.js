...
module.exports = async (client, reaction, user ) => {
    ...
    if (!user.bot) {
        ...
        await channel.send({ content: `<@${user.id}> a ajouté une réaction '👍' au message avec l'ID ${reaction.message.id} au message de <@${reaction.message.author.id}>.`, flags: MessageFlags.Ephemeral });
    }
};